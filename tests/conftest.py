...
def _discover_notebook():
    # If NOTEBOOK_PATH is set and exists, use it.
    ...
    # Otherwise, search the repo for .ipynb (skip checkpoints)
    cands = [
    ...
    # Prefer names that look like your main pipeline
    preferred = [p for p in cands if re.search(r"(pipeline|catboost|mlp)", p.name, re.I)]
    if len(preferred) == 1:
        return preferred[0]

    # If still ambiguous, pick the one with the most code cells
    ...
    top = cands[0]
    # If multiple tie, require explicit NOTEBOOK_PATH so we don't guess
